...
export const extractPrice = (priceString) => {
  ...
  // "600rs - 500G" -> 600
  // "Rs300/500G(Inc - all taxes)" -> 300

  const match = priceString.match(/Rs?(\d+)/i);
  if (match) {
    return parseFloat(match[1]);
  }

  // Fallback: extract first number
  const fallbackMatch = priceString.match(/(\d+)/);
...
export const calculateOrderTotal = (items) => {
  return items.reduce((total, item) => {
    ...
    console.log(`Item: ${item.name}, Price string: "${item.price}", Extracted price: ${price}, Quantity: ${item.quantity}`);
